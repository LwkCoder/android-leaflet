...
import com.lwkandroid.lib.common.widgets.view.iface.IClip;
import com.lwkandroid.lib.common.widgets.view.shadow.ShadowDrawable;
...
public class RBaseHelper<T extends View> implements IClip, ViewTreeObserver.OnGlobalLayoutListener
{
    ...
    private void setup()
    {
        ...
        if (useShadow())
        {
            mShadowDrawable = new ShadowDrawable();
        }
    ...
    private void setBackgroundState()
    {
        ...
        //是否自定义了背景
        if (unHasBgColor && unHasBgColorArray && unHasBgDrawable)
        {//未设置自定义背景
            hasCusBg = false;
        } else
        {
