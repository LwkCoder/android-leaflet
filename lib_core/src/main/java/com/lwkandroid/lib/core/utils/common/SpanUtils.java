...
public final class SpanUtils
{
    ...
    /**
     * 设置点击事件
     * <p>需添加 view.setMovementMethod(LinkMovementMethod.get())</p>
     *
    ...
    class CustomIconMarginSpan implements LeadingMarginSpan, LineHeightSpan
    {
        ...
        private Bitmap uri2Bitmap(final Uri uri)
        {
            try
            {
                return MediaStore.Images.Media.getBitmap(AppContext.get().getContentResolver(), uri);
            } catch (IOException e)
            {
                e.printStackTrace();
        ...
        {
            Drawable drawable = ContextCompat.getDrawable(AppContext.get(), resourceId);
    ...
    class CustomImageSpan extends CustomDynamicDrawableSpan
    {
        ...
        private CustomImageSpan(final Bitmap b, final int verticalAlignment)
        {
            ...
            mDrawable = new BitmapDrawable(AppContext.get().getResources(), b);
        ...
        public Drawable getDrawable()
        {
            ...
            } else if (mContentUri != null)
            {
                ...
                try
                {
                    InputStream is = AppContext.get().getContentResolver().openInputStream(mContentUri);
            ...
            } else
            {
                try
                {
                    drawable = ContextCompat.getDrawable(AppContext.get(), mResourceId);
                    drawable.setBounds(0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight());
                } catch (Exception e)
                {
